...
#else
   !$omp parallel do shared(itbl, at, zeff, dispm, gw, gam, q, g_a, g_c) &
...
!  get atomic reference contributions
   ...
   !$acc& dispm, dispm%nref, dispm%q, q, g_a, g_c)
#endif
...
function edisp_scc(dispm,nat,ndim,at,itbl,q,g_a,g_c,wdispmat,gw) result(ed)
   ...
   !$omp parallel do shared(zetavec, dispm, nat, itbl, at, zeff, gam, q, g_c, g_a) &
   !$omp private(k, ati, iz, ii)
   do iat = 1, nat
       k = itbl(iat)
       ati = at(iat)
